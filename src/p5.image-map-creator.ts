...
export class imageMapCreator {
  ...
  protected enableMapName: boolean;
  protected enableHistory: boolean;
  protected enableClear: boolean;
  protected enableGenerateHtml: boolean;
  protected enableGenerateSvg: boolean;
   ...
   * @param {number} height
   */
  //   width: number = 600, height: number = 450, onSave?: Function
  constructor(
    elementId: string,
    options: {
      width: number; //
    ...
    this.map = new ImageMap(this.width, this.height);
    ...
    // custom settings
    this.sidebarName = options.sidebarName || "Toolbar";
    this.enableTools = options.enableTools || ["polygon", "rectangle", "circle", "select", "delete", "test"];
    this.enableDefaultArea = typeof options.enableDefaultArea !== "undefined" ? options.enableDefaultArea : true;
    this.enableMapName = typeof options.enableMapName !== "undefined" ? options.enableMapName : true;
    this.enableHistory = typeof options.enableHistory !== "undefined" ? options.enableHistory : true;
    this.enableClear = typeof options.enableClear !== "undefined" ? options.enableClear : true;
    this.enableGenerateHtml = typeof options.enableGenerateHtml !== "undefined" ? options.enableGenerateHtml : true;
    this.enableGenerateSvg = typeof options.enableGenerateSvg !== "undefined" ? options.enableGenerateSvg : true;
    this.enableOutput = typeof options.enableOutput !== "undefined" ? options.enableOutput : true;
    this.enableSave = typeof options.enableSave !== "undefined" ? options.enableSave : true;

    let drawingTools: Tool[] = ["rectangle", "circle", "polygon"];
    this.drawingTools = drawingTools.filter((tool) => this.enableTools.includes(tool));

    this.tool = this.drawingTools.length > 0 ? this.drawingTools[0] : "polygon";
    ...
    if (typeof options.menuItems !== "undefined") {
      Object.keys(options.menuItems).forEach((key: string) => {
        if (typeof options.menuItems === "undefined") return;
        let menuItem = options.menuItems[key](this);
        this.menu[key] = menuItem;
      });
    }
  }
  ...
  private setup(): void {
    ...
    this.settings = QuickSettings.create(this.p5.width + 5, 0, this.sidebarName, this.p5.canvas.parentElement).setDraggable(false);

    if (this.enableMapName)
      this.settings.addText("Map Name", "", (v: string) => {
        this.map.setName(v);
      });
    ...
    if (this.enableDefaultArea)
      this.settings.addBoolean("Default Area", this.map.hasDefaultArea, (v: boolean) => {
        this.setDefaultArea(v);
      });

    if (this.enableHistory) {
      this.settings.addButton("Undo", this.undoManager.undo);
      this.settings.addButton("Redo", this.undoManager.redo);
    }

    if (this.enableClear) this.settings.addButton("Clear", this.clearAreas.bind(this));

    if (this.enableGenerateHtml)
      this.settings.addButton("Generate Html", () => {
        if (this.enableOutput) this.settings.setValue("Output", this.map.toHtml());
      });

    if (this.enableGenerateSvg)
      this.settings.addButton("Generate Svg", () => {
        if (this.enableOutput) this.settings.setValue("Output", this.map.toSvg());
      });

    if (this.enableOutput) this.settings.addTextArea("Output");

    if (this.enableSave) this.settings.addButton("Save", this.save.bind(this));
  ...
  handeFile(file: p5.File): void {
    if (file.type == "image") {
      ...
      if (!this.map.getName()) {
        ...
        if (this.enableMapName) this.settings.setValue("Map Name", this.map.getName());
      }
  ...
  drawImage(): void {
    if (this.img.data) this.p5.image(this.img.data, 0, 0, this.img.data.width, this.img.data.height);
  }
    ...
    switch (this.tool) {
      ...
      case "select":
        if (this.mouseIsHoverSketch()) {
          ...
          if (this.enableOutput) this.settings.setValue("Output", href);
        }
        break;
    }
  }
  ...
  setBackground(): void {
    ...
    if (!this.img.data) {
      ...
      let text = "Sleep hier een afbeelding naartoe";
  ...
  exportMap(): object {
    return new Save(version, this.map);
  }

  exportMapJson(): string {
    return JSON.stringify(this.exportMap(), function (key, value) {
  ...
  save(): void {
    if (this.onSave) {
      this.onSave({
        this: this,
        map: this.exportMap(),
        json: this.exportMapJson(),
      });
    } else {
      //@ts-ignore encoding options for Chrome
      let blob = new Blob([this.exportMapJson()], { encoding: "UTF-8", type: "text/plain;charset=UTF-8" });
  ...
  importMap(json: string): void {
    ...
    if (this.enableMapName) this.settings.setValue("Map Name", objectMap.name);
    if (this.enableDefaultArea) this.settings.setValue("Default Area", objectMap.hasDefaultArea);
    this.reset();
  }
  ...
  /**
   * Set the data of an area
   */
  setAreaData(area: Area, newData: Object): void {
    let oldData = area.getData();
    area.setData(newData);
    this.undoManager.add({
      undo: () => area.setData(oldData),
      redo: () => area.setData(newData!),
    });
  }
