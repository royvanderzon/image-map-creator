...
export class ImageMap {
   ...
   */
  constructor(public width: number, public height: number, protected areas: Area[] = [], protected name: string = "", public hasDefaultArea: boolean = false) {}
  ...
  addArea(area: Area, setId = true): number {
    if (setId) area.setId(this.getNewId());
  ...
  moveArea(id: number, direction: number): number | false {
    ...
    if (nextIndex < 0 || nextIndex >= this.areas.length) return false;
    this.rmvArea(id);
  ...
  getNewId(): number {
    ...
    let newId = Number(`${timestamp}${random}`);
    return newId;
  }
